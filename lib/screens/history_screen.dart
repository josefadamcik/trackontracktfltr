  ...
  ListView _buildListView(AsyncSnapshot<List<HistoryItem>> snapshot) {
    ...
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Padding(padding: EdgeInsets.all(8.0), child: Column(children: <Widget>[
            Row(children: <Widget>[
              Padding(padding: EdgeInsets.only(right: 8.0), child: Icon(item.icon)),
              Flexible(child: Text(item.title ?? "title n/a", style: theme.textTheme.title, overflow: TextOverflow.ellipsis, ))
            ],),
            Padding(padding: EdgeInsets.only(top: 8.0), child: Row(children: <Widget>[
              Flexible(child: Text(item.subtitle ?? "", style: theme.textTheme.subhead, overflow: TextOverflow.ellipsis, ),)
            ],)
            ),
            Padding(padding: EdgeInsets.only(top: 8.0), child:
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: <Widget>[
                Text(_typeInfoForItem(item),  style: theme.textTheme.caption),
                Text(_watchedAtFormat.format(item.watchedAt),
